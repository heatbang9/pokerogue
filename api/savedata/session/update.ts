...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (typeof req.body === "object" && req.body !== null) {
      sessionSaveData = req.body.session || req.body;
    } else if (typeof req.body === "string") {
      const params = new URLSearchParams(req.body);
      const sessionVal = params.get("session");
      if (sessionVal) {
        try {
          sessionSaveData = JSON.parse(sessionVal);
        } catch {
    ...
    if (!sessionSaveData) {
      return res.status(400).json({ status: "error", error: "No session data provided" });
    }

    await redis.set(`session:${session.username}:${slot}`, JSON.stringify(sessionSaveData));
