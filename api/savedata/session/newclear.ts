...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // Get query parameters (isVictory can be used for future validation)
    const { slot, isVictory } = req.query;
    void isVictory;
    ...
    if (!sessionSaveData) {
      // No session data means it's not cleared
      return res.status(200).json(false);
    }

    // Check if the session has been cleared (marked as newclear)
    const parsedSessionData = typeof sessionSaveData === "string" ? JSON.parse(sessionSaveData) : sessionSaveData;

    // Return whether this session is marked as cleared
    const isCleared = parsedSessionData.newClear === true || parsedSessionData.isVictory === true;

    return res.status(200).json(isCleared);
  } catch (error) {
    console.error("Newclear session savedata error:", error);
